function [dic, nc, v, effNum, posPatch] = updateDicSDA(iniPatches, grayImg, param, tmplsize, patchsize, category, MaxSubClassNum, numOfNegPatches)

% --------- get negative image blocks -----
negPatch = getNegImgBlocks(grayImg, param, tmplsize, patchsize, numOfNegPatches);

% --------- get positive image blocks -----
temppara = affparam2geom(param.est(:));
temppara = affparam2mat(temppara);
posImg   = warpimg(grayImg, temppara, tmplsize);
posPatch = getPosImgBlocls(posImg, patchsize);
posPatch = [iniPatches, posPatch];
...
low_cor         = v(:, 1:effNum)' * trainData(1:nc(1), :)';
paramSR.lambda2 = 0;
paramSR.mode    = 2;
